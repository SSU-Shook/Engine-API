...
const shellEscape = require('shell-escape');  // Add shell-escape library
...
module.exports = function getLockfile(packageFile, date, {
    ...
    // Sanitize date and only before using them
    let sanitizedDate = date ? shellEscape([date]) : '';
    let sanitizedOnly = only ? shellEscape([only]) : '';
